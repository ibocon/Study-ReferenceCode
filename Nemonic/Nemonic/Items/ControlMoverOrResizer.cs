...
namespace ControlManager
{
    internal class ControlMoverOrResizer
    {
        ...
        private static void CheckMouseTouchEdge(Control control, Point mouseLocationInControl)
        {
        ...
        private static void UpdateMouseCursorImage(Control control)
        {
            if (WorkType == MoveOrResize.Move)
            {
                return;
        ...
        private static void MoveControl(Control control, MouseEventArgs e)
        {
            ...
            {
                CheckMouseTouchEdge(control, new Point(e.X, e.Y));
                UpdateMouseCursorImage(control);
            }
            if (_resizing)
            {
                Rectangle newSize = new Rectangle(control.Left, control.Top, control.Width, control.Height);

                if (MouseIsInLeftEdge)
                {
                    if (MouseIsInTopEdge)
                    {
                        newSize.Width = control.Width - (e.X - _cursorStartPoint.X);
                        newSize.X = control.Left + (e.X - _cursorStartPoint.X);
                        newSize.Height = control.Height - (e.Y - _cursorStartPoint.Y);
                        newSize.Y = control.Top + (e.Y - _cursorStartPoint.Y);
                    }
                    else if (MouseIsInBottomEdge)
                    {
                        newSize.Width = control.Width - (e.X - _cursorStartPoint.X);
                        newSize.X = control.Left + (e.X - _cursorStartPoint.X);
                        newSize.Height = (e.Y - _cursorStartPoint.Y) + _currentControlStartSize.Height;
                    }
                    else
                    {
                        newSize.Width = control.Width - (e.X - _cursorStartPoint.X);
                ...
                else if (MouseIsInRightEdge)
                {
                    if (MouseIsInTopEdge)
                    {
                        newSize.Width = (e.X - _cursorStartPoint.X) + _currentControlStartSize.Width;
                        newSize.Height = control.Height - (e.Y - _cursorStartPoint.Y);
                        newSize.Y = control.Top + (e.Y - _cursorStartPoint.Y);
                ...
                int padding = parent.Padding.All;

                if (newSize.X >= padding)
                {
                    control.Left = newSize.X;
                    if (control.Left + newSize.Width <= parent.Width - padding)
                    {
                        control.Width = newSize.Width;
                    }
                }

                if (newSize.Y >= padding)
                {
                    control.Top = newSize.Y;
                    if (control.Top + newSize.Height <= parent.Height - padding)
                    {
            ...
            else if (_moving)
            {
                ...
                if (!_moveIsInterNal)
                {
                    ...
                    //X가 넘어가지는 않는지 체크
                    if (!(padding <= newLocation.X && (newLocation.X + control.Width) <= (parent.Width - padding)))
                    {
                        newLocation.X = control.Location.X;
                    }

                    //Y가 넘아가지 않는지 체크
                    if (!(padding <= newLocation.Y && (newLocation.Y + control.Height) <= (parent.Height - padding)))
                    {
