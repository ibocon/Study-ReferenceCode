...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, {key: getitem(A, key) + getitem(B, key) for key in A.f.keys() | B.f.keys()})
...
def scalar_mul(M, alpha):
    ...
    True
    """
    return Mat(M.D, {key: alpha * getitem(M, key) for key in M.f})
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), {(col, row): value for (row, col), value in M.f.items()})
...
def vector_matrix_mul(v, M):
    ...
    for (row, col) in M.f.keys():
        result_vec[col] += M[row, col] * v[row]
    return result_vec
...
def matrix_vector_mul(M, v):
    ...
    for (row, col) in M.f.keys():
        result_vec[row] += M[row, col] * v[col]
    return result_vec
