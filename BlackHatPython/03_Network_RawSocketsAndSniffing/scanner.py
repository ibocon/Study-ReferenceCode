...
# host to listen on
host = ni.get_ip_address(ni.get_networkinterfaces()[1])
# subnet to target
subnet = host+"/" + ni.get_netmask(ni.get_networkinterfaces()[1])
...
class IP(Structure):
    ...
    _fields_ = [
        ...
        ("sum",           c_ushort),
        # original code was "c_long" but, sizeof(C_long)=8bytes=64bits!=32bits
        # so change c_long(64bits) -> c_uint32(32bits)
        ("src",           c_uint32),
        ("dst",           c_uint32)
    ]
